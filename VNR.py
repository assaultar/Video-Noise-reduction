...
def denoise_seq_VNRnet(seq, noise_std, temp_psz, model_temporal):
	r"""Denoises a sequence of frames with VNRnet.
	...
	for fridx in range(numframes):
		if not inframes:
			for idx in range(temp_psz):
				relidx = abs(idx-ctrlfr_idx)
				inframes.append(seq[relidx])
		else:
			del inframes[0]
			relidx = min(fridx + ctrlfr_idx, -fridx + 2*(numframes-1)-ctrlfr_idx)
			inframes.append(seq[relidx])
